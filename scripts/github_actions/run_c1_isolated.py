#!/usr/bin/env python3
"""Run and verify one isolated GitHub Actions C1 pipeline execution."""
...
EXPECTED_BRANCH = "feature/policy-as-code-gates"
EXPECTED_ACCOUNT_ID = "522814714524"
EXPECTED_ROLE_NAME = "thesis-pac-dev-github-c1"
...
def safe_run_key(value: str) -> str:
    ...
    if not normalized:
        raise RuntimeError(
            "C1 run key is empty after normalization."
        )

    if len(normalized) > 80:
        raise RuntimeError(
            "C1 run key exceeds 80 characters."
        )
...
def shadow_inventory(
    *,
    ...
    if not data_objects:
        raise RuntimeError(
            "No isolated C1 data objects found."
        )
...
def athena_query_inventory(
    *,
    ...
    if failed:
        raise RuntimeError(
            "One or more isolated C1 Athena "
...
def execute(
    ...
    if not all(
        ...
        raise RuntimeError(
            "A shadow schema escaped the C1 prefix."
        )

    expected_data_root = (
        f"s3://{data_bucket}/experiments/c1/"
    )

    expected_results_root = (
        f"s3://{results_bucket}/experiments/c1/"
    )
    ...
    if not data_root_uri.startswith(
        ...
        raise RuntimeError(
            "C1 data root escaped its S3 boundary."
        )
