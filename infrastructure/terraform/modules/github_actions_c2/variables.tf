...
variable "github_repository" {
  description = "GitHub owner/repository allowed to assume the C2 role."
  type        = string
...
variable "github_branch" {
  description = "Exact GitHub branch allowed to assume the C2 role."
  type        = string
...
variable "bronze_database_name" {
  description = "Canonical Bronze Glue database read by C2."
  type        = string
}
...
variable "shadow_database_prefix" {
  description = "Required prefix for isolated C2 Glue databases."
  type        = string
  default     = "thesis_pac_c2_"
...
variable "c2_fallback_state_machine_arn" {
  ...
  validation {
    condition = can(
      regex(
        "^arn:[^:]+:states:[a-z0-9-]+:[0-9]{12}:stateMachine:[A-Za-z0-9_-]+$",
        var.c2_fallback_state_machine_arn
      )
    )

    error_message = "c2_fallback_state_machine_arn must be a Step Functions state-machine ARN."
  }
}
