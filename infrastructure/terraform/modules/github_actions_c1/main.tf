...
locals {
  ...
  shadow_data_object_arn = (
    "${var.data_lake_bucket_arn}/experiments/c1/*"
  )

  shadow_result_object_arn = (
    "${var.athena_results_bucket_arn}/experiments/c1/*"
  )
}
...
resource "aws_iam_role" "c1" {
  name = "${local.name_prefix}-github-c1"

  description = (
    "Immutable repository and branch scoped GitHub Actions role for isolated C1 Policy-as-Code validation."
  )
...
data "aws_iam_policy_document" "c1" {
  statement {
  ...
  statement {
    sid    = "ListBronzeAndC1DataPrefixes"
    effect = "Allow"
  ...
  statement {
    sid    = "ManageIsolatedC1DataObjects"
    effect = "Allow"
  ...
  statement {
    sid    = "ListIsolatedC1AthenaResults"
    effect = "Allow"
  ...
  statement {
    sid    = "ManageIsolatedC1AthenaResults"
    effect = "Allow"
  ...
  statement {
    sid    = "CreateIsolatedC1GlueDatabases"
    effect = "Allow"
  ...
  statement {
    sid    = "ManageIsolatedC1GlueDatabases"
    effect = "Allow"
  ...
  statement {
    sid    = "ManageIsolatedC1GlueTables"
    effect = "Allow"
...
resource "aws_iam_policy" "c1" {
  name = "${local.name_prefix}-github-c1"

  description = (
    "Least-privilege Bronze read and isolated C1 Glue, S3 and Athena permissions."
  )

  policy = data.aws_iam_policy_document.c1.json
}

resource "aws_iam_role_policy_attachment" "c1" {
  role       = aws_iam_role.c1.name
  policy_arn = aws_iam_policy.c1.arn
}
